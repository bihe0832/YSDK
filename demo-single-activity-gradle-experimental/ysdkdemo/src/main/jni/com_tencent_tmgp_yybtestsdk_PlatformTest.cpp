...
public:
        ...
        if(loginRet.ret == RET_SUCC){
            ...
            if (loginRet.platform == ePlatform_QQ) {
                ...
                switch (loginRet.flag) {
                    case eFlag_User_QQ_NotInstall:
                    case eFlag_User_QQ_NotSupportApi:
                    case eFlag_User_QQ_UserCancel:
                    case eFlag_User_QQ_NoAcessToken:
                    case eFlag_User_QQ_LoginFail:
                    case eFlag_User_QQ_NetworkErr:
						method = sGlobalJNIEnv->GetMethodID(cls, "letUserLogout", "()V");
						sGlobalJNIEnv->CallVoidMethod(sGlobalActivity, method);
                ...
                switch (loginRet.flag) {
                    case eFlag_User_WX_NotSupportApi:
                    case eFlag_User_WX_UserCancel:
                    case eFlag_User_WX_UserDeny:
                    case eFlag_User_WX_AccessTokenExpired:
                    case eFlag_User_WX_RefreshTokenExpired:
                    case eFlag_User_WX_LoginFail:
                    case eFlag_Error:
                    case eFlag_User_WX_RefreshTokenFail:
						method = sGlobalJNIEnv->GetMethodID(cls, "letUserLogout", "()V");
						sGlobalJNIEnv->CallVoidMethod(sGlobalActivity, method);
						LOGD("OnLoginNotify call letUserLogout end%s", "");
                        break;
                    case eFlag_User_WX_RefreshTokenSucc: {
                        // 下面是MSDKSample使用的逻辑, 游戏忽略此部分内容
                        method = sGlobalJNIEnv->GetMethodID(cls, "letUserLogin", "()V");
                        sGlobalJNIEnv->CallVoidMethod(sGlobalActivity, method);
                        LOGD("OnLoginNotify call letUserLogin end%s", "");
	...
	virtual void OnWakeupNotify(WakeupRet& wakeupRet){
        ...
        switch(wakeupRet.flag){
            case eFlag_User_UrlLogin:{
                // 下面是MSDKSample使用的逻辑, 游戏忽略此部分内容
                LOGD("OnWakeupNotify: platform:%s","");
                break;
            }
            case eFlag_User_NeedSelectAccount:{
                LOGD("diff account%s", "");
                jclass cls = sGlobalJNIEnv->GetObjectClass(sGlobalActivity);
	...
	virtual void OnRelationNotify(UserRelationRet& relationRet){
        ...
        if(relationRet.ret == RET_SUCC){
            for (int i = 0; i < relationRet.persons.size(); i++) {
				...
				result << "is_yellow_vip:" <<relationRet.persons.at(i).is_yellow_vip << "\n";
				result << "is_yellow_year_vip:" <<relationRet.persons.at(i).is_yellow_year_vip << "\n";
				result << "yellow_vip_level:" <<relationRet.persons.at(i).yellow_vip_level << "\n";
				result << "is_yellow_high_vip:" <<relationRet.persons.at(i).is_yellow_high_vip << "\n";
            }
        }
...
public:
	virtual void OnPayNotify(PayRet& ret){
		...
		if(RET_SUCC == ret.ret){
			...
			switch (ret.flag){
				case eFlag_User_LocalTokenInvalid:
					//用户取消支付
					desc =  "用户登录态失效，请重新登录";
					result << desc.c_str() << "\n";
					break;
